 ...
 */
public class LiquichainIBFTBlockTransactionSelector {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(LiquichainIBFTBlockTransactionSelector.class);
  ...
  public LiquichainIBFTBlockTransactionSelector(
  ...
  public TransactionSelectionResults buildTransactionListForBlock() {
    ...
    pendingTransactions.selectTransactions(
        pendingTransaction -> {
          final var res = evaluateTransaction(pendingTransaction, false, true);
  ...
  public TransactionSelectionResults evaluateTransactions(final List<Transaction> transactions) {
    transactions.forEach(
        transaction ->
            transactionSelectionResults.addSelectionResult(evaluateTransaction(transaction, true, false)));
    return transactionSelectionResults;
  }
  ...
  private TransactionSelectionResult evaluateTransaction(
      final Transaction transaction,
      final boolean reportFutureNonceTransactionsAsInvalid,
      final boolean checkIdenticalTo) {
    if (isCancelled.get()) {
      throw new CancellationException("Cancelled during transaction selection.");
    }

    if (checkIdenticalTo && !transactionWithIdenticalTo(transaction)) {
  ...
  private boolean transactionWithIdenticalTo(Transaction transaction) {

    List<Transaction> results = transactionSelectionResults.transactions;
    if (results.size() == 0) return true;
    var lastIndex = results.size() - 1;
    Transaction prevTransaction = results.get(lastIndex);
    Optional<Account> transactionTo = Optional.empty();
    Optional<Account> prevTransactionTo = Optional.empty();
    if (transaction.getTo().isPresent()) {
      transactionTo = Optional.ofNullable(worldState.get(transaction.getTo().get()));

    }
    if (prevTransaction.getTo().isPresent()) {
      prevTransactionTo = Optional.ofNullable(worldState.get(prevTransaction.getTo().get()));
    }

    if (transactionTo.isPresent() && prevTransactionTo.isPresent()) {
      if (!prevTransactionTo.get().hasCode() && !transactionTo.get().hasCode()) {
        return true;
      }
      if (prevTransactionTo.get().hasCode() || transactionTo.get().hasCode()) {
        return prevTransaction.getTo().get() == transaction.getTo().get();
      }
    }
